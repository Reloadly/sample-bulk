    ...
    <tbody>
        @foreach($invoice['topups'] as $topup)
            <tr>
                ...
                <td>{{ number_format($invoice['amount']/sizeof($invoice['topups']),2).' '.$invoice['currency']['code'] }}</td>
            </tr>
        @endforeach
    </tbody>
</table><br>
<br>
SUBTOTAL : {{$invoice['amount'].' '.$invoice['currency']['code']}}<br>
DISCOUNT (0%) : 0.00 {{ $invoice['currency']['code'] }}<br>
TOTAL : {{$invoice['amount'].' '.$invoice['currency']['code']}}<br>
<br>
<br>
Kind Regards,<br>
